...
import { clearSession, getMedico, type MedicoSession } from "@/lib/recetario/auth";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!session.validado || !session.matricula_validada) {
      router.replace("/recetario/cuenta-en-revision");
      return;
    }
    if (!session.firma_url) {
      router.replace("/recetario/firma-digital");
      return;
    }
    setMedico(session);
  }, [router]);
  ...
  return (
    ...
    >
              ...
              return (
                <Link
                  ...
                  style={{
                    ...
                    border: active
                      ? "1px solid rgba(10,230,199,0.22)"
      ...
      <main style={{ flex: 1, paddingTop: "5.5rem", paddingBottom: "4rem" }}>
      ...
      >
        ...
        © 2026 DocYa | Documentos medicos digitales
      </div>
    </div>
  );
}
