...
export default function DownloadApp() {
  return (
    <section id="descargar" className="relative overflow-hidden bg-[var(--brand)]">
      {/* capas de fondo — z-0 (no negativas) */}
      ...
      <div className="relative z-10 container px-6 py-20 md:py-24 text-center text-white">
        ...
        <motion.div
          ...
          transition={{ duration: 0.45, ease: cubicBezier(0.22, 1, 0.36, 1) }}
        >
          <span className="badge bg-white/10 text-white border-white/20">Profesionales verificados</span>
          <span className="badge bg-white/10 text-white border-white/20">Atención en &lt; 35 min</span>
          <span className="badge bg-white/10 text-white border-white/20">Pagos seguros</span>
        </motion.div>
      </div>

      {/* ornamentos — z-0 */}
      <div className="pointer-events-none absolute -bottom-24 -right-20 h-72 w-72 rounded-full opacity-20 blur-2xl z-0" style={{ background: "radial-gradient(closest-side, white, transparent)" }} />
      <div className="pointer-events-none absolute -top-16 -left-24 h-60 w-60 rounded-full opacity-10 blur-2xl z-0" style={{ background: "radial-gradient(closest-side, white, transparent)" }} />
    </section>
  );
}
