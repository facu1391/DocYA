...
export default function InvitarAmigosScreen() {
  ...
  return <main style={{ minHeight: "100vh", background: "#061a22", color: "#f8fafc", padding: "24px 18px 70px", fontFamily: "system-ui" }}>
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      ...
      <div style={{ textAlign: "center", padding: "30px 0 22px" }}><Gift size={44} color="#a5b4fc"/><h1>Invitá amigos</h1><p style={{ color: "#94a3b8" }}>Ganás una teleconsulta gratis cada 5 amigos que completen su primera teleconsulta.</p></div>
      <section style={{ background: "linear-gradient(135deg,#172554,#0f766e)", borderRadius: 24, padding: 24 }}>
        ...
        <p>{summary.progress_in_cycle === 0 ? "Compartí tu enlace para empezar" : `Te faltan ${remaining} amigos para tu próxima teleconsulta gratis`}</p>
      ...
      <section style={{ marginTop: 18 }}><h2>Amigos invitados</h2>{friends.length === 0 ? <p style={{ color: "#94a3b8" }}>Todavía no hay amigos registrados con tu enlace.</p> : friends.map(f => <div key={f.id} style={{ display: "flex", gap: 12, alignItems: "center", padding: 15, marginBottom: 9, background: "#0b2732", borderRadius: 15 }}>{f.status === "qualified" ? <CheckCircle2 color="#2dd4bf"/> : <Clock3 color="#fbbf24"/>}<div><strong>{f.display_name}</strong><div style={{ color: "#94a3b8", fontSize: 13 }}>{f.status === "qualified" ? "Referido válido" : "Pendiente de primera teleconsulta"}</div></div></div>)}</section>
    </div>
  </main>;
}
